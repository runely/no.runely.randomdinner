...
const getIngredients = (app, includePredefinedFood, includePredefinedNonsense, includePredefinedToiletWords, ownFood, ownNonsense, ownToiletWords, language) => {
  const foods = []
  ...
  app.log(`Including ${ownNonsense.length} custom nonsense (if not already included)`)
  ownNonsense.forEach(own => {
    if (!foods.includes(own)) {
      foods.push(own)
...
module.exports = (app, ingredientCount, includePredefinedFood, includePredefinedNonsense, includePredefinedToiletWords, ownFood, ownNonsense, ownToiletWords, allowIngredientMultipleTimes, language) => {
  // Reset used food
  usedFood = []

  const cookingMethods = getTranslation('cookingmethod', language).values

  const ingredients = getIngredients(app, includePredefinedFood, includePredefinedNonsense, includePredefinedToiletWords, ownFood, ownNonsense, ownToiletWords, language)
  ...
  const randomCookedFoods = getRandomFood(app, 1, ingredients, true)
  const randomCookedFood = randomCookedFoods.length > 0 ? randomCookedFoods[0] : ''
